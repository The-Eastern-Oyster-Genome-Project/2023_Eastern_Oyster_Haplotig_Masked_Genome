...
#BWA for mapping for all samples.  As of version 2.0 can handle SE or PE reads by checking for PE read files
        	...
        	if [ -f "$i.R2.fq.gz" ]; then
        		bwa mem -L 20,5 -I $INSERT,$SD,$INSERTH,$INSERTL -t $NUMProc -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" reference.fasta $i.R1.fq.gz $i.R2.fq.gz  2> bwa.$i.log | mawk '$6 !~/[2-9].[SH]/ && $6 !~ /[1-9][0-9].[SH]/' | samtools view -@$SAMProc -SbT reference.fasta - > $i.bam 2>$i.bam.log
        	else
        		bwa mem -L 20,5 -t $NUMProc -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" reference.fasta $i.R1.fq.gz 2> bwa.$i.log | mawk '$6 !~/[2-9].[SH]/ && $6 !~ /[1-9][0-9].[SH]/' | samtools view -@$SAMProc -SbT reference.fasta - > $i.bam 2>$i.bam.log
        	fi
        	...
        	do
        	if [ -f "$i.R2.fq.gz" ]; then
        		bwa mem -t $NUMProc -M -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" reference.fasta $i.R1.fq.gz $i.R2.fq.gz  2> bwa.$i.log | samtools sort -@$NUMProc -o $i-RG.bam 2>$i.bam.log
        	else
        		bwa mem -t $NUMProc -M -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" reference.fasta $i.R1.fq.gz  2> bwa.$i.log | samtools sort -@$NUMProc -o $i-RG.bam  2>$i.bam.log
        	fi
		samtools index $i-RG.bam
        	done
        fi
	...
	cat namelist | parallel -j $NUMProc "rm {}-RG.bam"
	filter_bam(){
	samtools view -@32 -h -q 10 -F 0x100 -F 0x400 $1-RGmd.bam | mawk '$6 !~/[8-9].[SH]/ && $6 !~ /[1-9][0-9].[SH]/'| samtools view -@ 32 -b
	}

	export -f filter_bam
	cat namelist | parallel --env filter_bam -j 8 "filter_bam {} > {}.F.bam"
fi
...
if [ "$SNP" != "no" ]; then
	...
	if [[ "$MAP" == "no" && ! -f "mapped.bed" ]]; then
		...
		p_bam_depth(){ samtools depth $1 | mawk '{if ($3 > 1); print $1 "\t" $2-1 "\t" $2}' | bedtools merge -i -; }
		export -f p_bam_depth
		FB2=$(( $NUMProc / 2 ))
		ls *.F.bam | parallel --env p_bam_depth -j $FB2 "p_bam_depth {}" | sort -S 2G --parallel=$NUMProc -k1,1 -k2,2n | bedtools merge -i - > mapped.bed
	fi
	...
	#if [ "filter.merged.bam" -nt "cov.stats" ]; then
	#	FB2=$(( $NUMProc / 2 ))
	#	if [ "$BEDTOOLSFLAG" == "OLD" ]; then
	#		cat namelist | parallel -j $FB2 "coverageBed -abam {}.F.bam -b mapped.bed -counts > {}.cov.stats"
	#	else
	#		mawk -v OFS='\t' {'print $1,$2'} reference.fasta.fai > genome.file
	#		cat namelist | parallel -j $FB2 "bedtools coverage -b {}.F.bam -a mapped.bed -counts -sorted -g genome.file > {}.cov.stats"
	#	fi
	...
	#mawk '$4 >  1000000' cov.stats > cov.high.stats
	#mawk '$4 <= 1000000' cov.stats > cov.low.stats
	...
	PFiles=$(( $GENOME / 100000 ))
	split -n l/$PFiles -a 5 --additional-suffix=.bed -d mapped.bed mapped.	
...
###New implementation of SNP calling here to save on memory	
	call_genos(){
		...
		freebayes -b filter.merged.bam -t mapped.$1.bed -f reference.fasta -m 5 -q 5 -E 3 --min-repeat-entropy 1 --populations popmap -n 10 -F 0.1 | bcftools view -O b -o raw.$1.bcf &> fb.$1.error.log
		if [ $? -eq 0 ]; then
	...
	export -f call_genos
      	call_genos2(){
                ...
                #samtools index split.$1.bam
		echo "Starting Freebayes instance " $1 >> freebayes.log
                freebayes -b filter.merged.bam -t mapped.$1.bed -v raw.$1.vcf -f ../reference.fasta -m 5 -q 5 -E 3 --min-repeat-entropy 1 --populations ../popmap -n 10 -F 0.1 | bcftools view -O b -o raw.$1.bcf
                if [ $? -eq 0 ]; then
                        rm mapped.$1.bed
                else
                        echo -e "$?" "\t" "$1" >> freebayes.error
		...
		if [ -f "freebayes.error" ]; then
                        ...
                        ERROR3=0
                        export ERROR3
                        mv raw.1.bcf raw.01.bcf 2>/dev/null
                        mv raw.2.bcf raw.02.bcf 2>/dev/null
                        mv raw.3.bcf raw.03.bcf 2>/dev/null
                        mv raw.4.bcf raw.04.bcf 2>/dev/null
			bcftools concat --naive raw.01.bcf raw.02.bcf raw.03.bcf raw.04.bcf -O b | bcftools view -O v| sed -e 's/	\.\:/	\.\/\.\:/g' > RedoRawSNPs.vcf
                        #bcftools  raw.*.vcf | sed -e 's/       \.\:/   \.\/\.\:/g' > RedoRawSNPs.vcf
	...
	seq 0 $NumK | parallel "ls raw.0{}*.bcf > bcf.{}.list"
	seq 0 $NumK | parallel "bcftools concat -n -f bcf.{}.list -O b -o Collated.raw.{}.bcf"
	#seq 0 $NumK | parallel "vcfcombine raw.0{}*.vcf | sed -e 's/    \.\:/   \.\/\.\:/g' > Collated.raw.{}.vcf"

	ls Collated.raw.*.bcf > collated.bcf.list
	bcftools concat -n -f collated.bcf.list -O b | bcftools view -O z -o  TotalRawSNPs.vcf.gz
	#vcfcombine Collated.raw.*.vcf > TotalRawSNPs.vcf	
	...
	vcftools --gzvcf TotalRawSNPs.vcf.gz $VCFGTFLAG 0.9 --out Final --non-ref-af 0.001 --max-non-ref-af 0.9999 --mac 1 --minQ 30 &>VCFtools.log
fi
